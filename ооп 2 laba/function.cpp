...
Mixture::Mixture(ifstream& file)//изучить эту тему
{
	...
	return (1 / pow(Dksi_Mixture(), 3 / 2)) *
		((1 - p) *
			(pow(HD1->Mksi_huber() - Mksi_mixture(), 3) + 3 *
				...
				pow(HD1->Dksi_huber(), 3 / 2) * HD1->Mksi_huber()) +
			(p) *
...
vector<double> Empirical::generate_x_selection() const
{
...
Empirical::Empirical(const HuberD* HD, int _n, int _k)//генерирование выборки по хьюберу
{
	...
	n = _n;
	k = _k;
	x_selection = HD->selection(_n);
	f_selection = generate_f_selection();
}

Empirical::Empirical(const Mixture* MD, int _n, int _k)//генерирование выборки по смеси
{
	if (_n <= 1) {
		throw invalid_argument("Некорректный аргумент");
	}
	...
	n = _n;
	k = (_k > 2) ? _k : static_cast<int>(floor(log2(_n))) + 1;
	x_selection = MD->selection_mixture(_n);
	f_selection = generate_f_selection();
}
...
Empirical::Empirical(const Empirical* ED)//конструктор копирования
{
	...
	n = ED->n;
	k = (ED->k > 2) ? ED->k : static_cast<int>(floor(log2(ED->n))) + 1;
...
Empirical::Empirical(const int _n, const int _k)//генерирование выборки по эмпирическому распределению
{
	...
	n = _n;
	k = (_k > 2) ? _k : static_cast<int>(std::floor(std::log2(_n))) + 1;
	x_selection = generate_x_selection();
	f_selection = generate_f_selection();
}

Empirical::Empirical(const vector<double>& x_selection)
	: n(x_selection.size()), k(static_cast<int>(std::floor(std::log2(x_selection.size()))) + 1)
{
...
Empirical& Empirical::operator=(const Empirical& ED)
{
	if (this == &ED)
		return *this;
...
{
	const double expected_val = Mn();
	const double variance_val = Dn();
	double sum = 0;
	...
	for (int i = 0; i < k; i++)
	{
		if (x_min + delta * i <= x && x < x_min + delta * (i + 1))
		{
			int n_i = count_if(x_selection.begin(), x_selection.end(), //считаем кол во элементов в x_selection
				...
				{ return i == k - 1 ? x_min +
				delta * (double)i <= x && x <= x_min + delta * (double)(i + 1) : x_min + delta * (d
	double sum = 0;
	for (double& x : x_selection)
		sum += x;
	return sum / n;
...
{
	vector<double> f_s;ouble)i <= x && x < x_min +
				delta * (double)(i + 1); });
			return n_i / (n * delta);
		}
	}
